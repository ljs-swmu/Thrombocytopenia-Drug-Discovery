...
from skopt.space import Real, Integer, Categorical  # Import Real, Integer, and Categorical
...
def main(input_file, output_file, y_col, X1_col, X2_col):
    ...
    # Parameter space for LogisticRegression
    param_space = {
        'C': Real(1e-6, 1e3, prior='log-uniform'),  # Regularization parameter
        'solver': Categorical([ 'liblinear']),  # Solver for optimization
        'max_iter': Integer(100, 1000),  # Maximum number of iterations
        'penalty': Categorical(['l1', 'l2'])  # Penalty (regularization) term
    }

    # Initialize default model (LogisticRegression)
    ...
    # Initialize model and Bayesian optimization (LogisticRegression)
    ...
    best_model = LogisticRegression(**bayes_search.best_params_, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Logistic Regression Classifier with Bayesian Optimization')
