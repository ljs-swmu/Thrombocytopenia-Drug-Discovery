...
def main(input_file, output_file, y_col, X1_col, X2_col):
    ...
    # Parameter space for AdaBoostClassifier
    param_space = {
        'n_estimators': Integer(2, 200),  # Number of weak learners
        'learning_rate': Real(0.01, 1.0, prior='log-uniform'),  # Learning rate
    }

    # Initialize default model (AdaBoostClassifier)
    ...
    # Initialize model and Bayesian optimization (AdaBoostClassifier)
    ...
    best_model = AdaBoostClassifier(random_state=42, **bayes_search.best_params_)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='AdaBoost Classifier with Bayesian Optimization')
